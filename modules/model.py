...
class MicrobiomeTransformer(nn.Module):
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        embeddings_type1 = batch[
            "embeddings_type1"
        ]  # (batch_size, seq_len1, input_dim_type1)
        embeddings_type2 = batch[
            "embeddings_type2"
        ]  # (batch_size, seq_len2, input_dim_type2)
        mask = batch["mask"]  # (batch_size, total_seq_len)
        type_indicators = batch[
            "type_indicators"
        ]  # (batch_size, total_seq_len) - 0 for type1, 1 for type2

        # Project each type separately
        x1 = self.input_projection_type1(
            embeddings_type1
        )  # (batch_size, seq_len1, d_model)
        x2 = self.input_projection_type2(
            embeddings_type2
        )  # (batch_size, seq_len2, d_model)
        ...
        )  # (batch_size, total_seq_len, d_model)
...
if __name__ == "__main__":
    model = MicrobiomeTransformer(
        input_dim_type1=384, input_dim_type2=256, d_model=512, nhead=8, num_layers=6
    )
    ...
    print(
        f"Type 2 output shape: {output[:, seq_len1 : seq_len1 + seq_len2].shape}"
    )  # (4, 40)
    ...
    print(
        f"Active output range: {active_output.min().item():.3f} to {active_output.max().item():.3f}"
    )
