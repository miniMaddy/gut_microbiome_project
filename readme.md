...
2.  **Classifier Head:** The extracted embeddings are then fed into a downstream classifier such as **Logistic Regression**, **Random Forest**, **Decision Tree**, **SVM**, or **MLP** to perform the final binary prediction.
...
model:
  # Classifier type: "logreg", "rf", "dt", "svm", or "mlp"
  classifier: "logreg"
  use_scaler: true
...
python main.py datasets=gadir model.classifier=[logreg,rf,dt,svm,mlp]
```
...
```bash
python main.py datasets=diabimmune \
  model.classifier=[logreg,rf,dt,svm,mlp] \
